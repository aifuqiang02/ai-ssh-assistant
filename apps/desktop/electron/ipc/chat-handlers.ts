...
export function registerChatHandlers() {
  ...
  // 更新会话
  ipcMain.handle('chat:update-session', async (event, userId: string, sessionId: string, data: any) => {
    try {
  ...
  // 发送消息
  ipcMain.handle('chat:send-message', async (event, userId: string, sessionId: string, content: string) => {
    try {
