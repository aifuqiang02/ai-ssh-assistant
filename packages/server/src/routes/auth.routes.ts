...
export async function authRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: WechatLoginBody }>('/wechat-login', {
    schema: {
      description: '微信扫码登录后换取业务系统登录态',
      tags: ['认证'],
      body: {
        type: 'object',
        required: ['bizId', 'appId', 'openId', 'nickname'],
        properties: {
    ...
    try {
      const { bizId, appId, openId, unionId, nickname, avatarUrl } = request.body

      let user = await userService.findUserByWechatIdentity(openId, unionId)
      ...
      if (!user) {
        ...
        user = await userService.updateWechatProfile(user.id, {
          nickname,
          avatarUrl,
          openId,
          unionId
