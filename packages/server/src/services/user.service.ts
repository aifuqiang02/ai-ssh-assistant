...
import { PrismaClient, User, UserRole } from '../../../database/src/generated/client/index.js'
...
export interface CreateWechatUserData {
  nickname: string
  avatarUrl?: string
  openId: string
  unionId?: string
...
export class UserService {
  ...
  async findUserByWechatIdentity(openId?: string, unionId?: string): Promise<User | null> {
    try {
      if (unionId) {
        const unionUser = await this.prisma.user.findFirst({
          where: { wechatUnionId: unionId } as any
        })
        if (unionUser) {
          return unionUser
        }
      ...
      logger.error('Failed to find user by wechat identity:', error)
      throw error
    }
  }

  async createWechatUser(data: CreateWechatUserData): Promise<User> {
    try {
      const baseName = data.nickname?.trim() || '微信用户'
      let username = baseName.replace(/\s+/g, '')

      if (!this.validateUsernameFormat(username).isValid) {
        username = `wx_${Date.now().toString(36)}`
      }

      let suffix = 0
      let candidate = username
      while (await this.isUsernameExists(candidate)) {
        suffix += 1
      ...
      logger.error('Failed to create WeChat user:', error)
      throw error
    }
  }
  ...
  async updateWechatProfile(
    userId: string,
    data: { nickname: string; avatarUrl?: string; openId: string; unionId?: string }
  ): Promise<User> {
    try {
      return await this.prisma.user.update({
        ...
        data: {
          ...
          wechatOpenId: data.openId,
          wechatUnionId: data.unionId || undefined
        } as any
      })
    } catch (error) {
      logger.error('Failed to update WeChat profile:', error)
      throw error
    }
  }
  ...
  async getUserStats(userId: string) {
    try {
      const [sshConnectionsCount, commandLogsCount] = await Promise.all([
