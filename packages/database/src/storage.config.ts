...
export const storagePresets = {
  // 完全本地模式（隐私优先）
  // 注意：在 Electron 环境中，应该由主进程提供用户数据目录的完整路径
  localOnly: {
    mode: 'local' as const,
    localOptions: {
      connectionString: 'file:./local.db', // Electron 会在主进程中覆盖为用户数据目录路径
      enabled: true
    },
    cloudOptions: {
      enabled: false
  ...
  highPerformance: {
    ...
    localOptions: {
      connectionString: 'file:./cache.db', // Electron 会在主进程中覆盖为用户数据目录路径
      enabled: true
    },
    cloudOptions: {
      connectionString: getEnv('DATABASE_URL'),
  ...
  mobile: {
    mode: 'hybrid' as const,
    localOptions: {
      connectionString: 'file:./mobile.db', // Electron 会在主进程中覆盖为用户数据目录路径
      enabled: true
    },
    cloudOptions: {
      connectionString: getEnv('DATABASE_URL'),
